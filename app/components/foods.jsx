...
export const Foods = ({selectedFoods, setSelectedFoods}) => {
    ...
    let divs = foods.map(function(item, index, array){
        return <div key={index} className={selectedFoods.includes(item) 
            ...
            : `bg-slate-50 select-none w-50p rounded-lg min-h-[50px] hover:bg-slate-100 hover:shadow-md border-2 border-transparent hover:border-lime-800 transition-all duration-200 ease-in-out cursor-pointer box-border`
    ...
    return (
        <div>
            <div className="grid grid-cols-4 gap-2 mx-60 rounded-2xl border-lime-800 border-2 p-2">
                {divs}
            </div>
         </div>
    )
